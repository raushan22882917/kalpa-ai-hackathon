...
export class AppInstallationService extends EventEmitter {
     ...
     */
    async saveAppPackage(deviceId, fileName, fileData) {
     ...
     */
    async installApp(deviceId, platform, filePath, onProgress) {
     ...
     */
    async launchApp(deviceId, platform, packageName) {
        try {
            if (platform === 'android') {
                // Launch Android app using ADB
                const result = await this.adbBridge.executeCommand(deviceId, `monkey -p ${packageName} -c android.intent.category.LAUNCHER 1`);
                ...
                // Launch iOS app
                const result = await this.iosBridge.executeCommand(deviceId, `ios-deploy --bundle_id ${packageName} --justlaunch`);
                if (result.exitCode !== 0) {
                    throw new Error(`Failed to launch app: ${result.stderr}`);
                }
            }
     ...
     */
    async startLogCapture(deviceId, platform, packageName, onLog) {
     ...
     */
    stopLogCapture(deviceId, packageName) {
        const streamKey = `${deviceId}_${packageName}`;
     ...
     */
    startAndroidLogCapture(deviceId, packageName, onLog, streamKey) {
        ...
        const interval = setInterval(async () => {
            try {
                const result = await this.adbBridge.executeCommand(deviceId, `logcat -d -s ${packageName}:* --format time`);
     ...
     * Start iOS log capture
     */
    startIOSLogCapture(deviceId, packageName, onLog, streamKey) {
        // In a real implementation, this would use idevicesyslog or similar
        const interval = setInterval(async () => {
            try {
                const result = await this.iosBridge.executeCommand(deviceId, `idevicesyslog -m ${packageName}`);
                if (result.exitCode === 0 && result.stdout) {
     ...
     */
    parseAndroidLogs(output, packageName) {
        const logs = [];
     ...
     */
    parseIOSLogs(output, packageName) {
        const logs = [];
        const lines = output.split('\n');
        for (const line of lines) {
            if (!line.trim())
     ...
     */
    async extractPackageName(platform, filePath) {
