 ...
 */
router.get('/:deviceId/list', async (req, res) => {
    try {
        ...
        const entries = await fileSystemService.listDirectory(deviceId, path, device.platform);
        res.json({
            success: true,
            path,
            entries,
...
router.get('/:deviceId/info', async (req, res) => {
    try {
        ...
        const fileInfo = await fileSystemService.getFileInfo(deviceId, path, device.platform);
        res.json({
            success: true,
            fileInfo
        });
...
router.post('/:deviceId/upload', async (req, res) => {
    try {
        ...
        const result = await fileSystemService.uploadFile(deviceId, localPath, devicePath, device.platform);
        if (result.success) {
            res.json({
                success: true,
 ...
 */
router.get('/:deviceId/download', async (req, res) => {
    try {
        const { deviceId } = req.params;
        const { path } = req.query;
        if (!path) {
        ...
        const fileBuffer = await fileSystemService.downloadFile(deviceId, path, device.platform);
...
router.delete('/:deviceId/delete', async (req, res) => {
    try {
        ...
        const result = await fileSystemService.deleteFile(deviceId, path, device.platform);
        if (result.success) {
            res.json({
                success: true,
...
router.put('/:deviceId/rename', async (req, res) => {
    try {
        ...
        const result = await fileSystemService.renameFile(deviceId, oldPath, newPath, device.platform);
        if (result.success) {
            res.json({
                success: true,
 ...
 */
router.post('/:deviceId/mkdir', async (req, res) => {
    try {
        const { deviceId } = req.params;
        const { path } = req.body;
        if (!path) {
        ...
        const result = await fileSystemService.createDirectory(deviceId, path, device.platform);
        if (result.success) {
            res.json({
                success: true,
 ...
 */
router.delete('/:deviceId/rmdir', async (req, res) => {
    try {
        const { deviceId } = req.params;
        ...
        const result = await fileSystemService.deleteDirectory(deviceId, path, recursive, device.platform);
        if (result.success) {
            res.json({
                success: true,
